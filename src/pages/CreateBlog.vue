    ...
    <div class="container mt-5">
      ...
      <div class="row align-items-center pl-3 mt-3">
        <div v-for="(item, index) in tags" :key="index" class="form-check d-inline mr-3">
          <input class="form-check-input" type="checkbox" :value="item.tag" :id="item.tag" v-model="chosenTags" />
      ...
      <div class="row mt-3 pl-3">
        <div v-for="(item, index) in categories" :key="index" class="form-check form-check-inline">
          <input class="form-check-input" type="radio" :value="item" :id="item" v-model="chosenCategory" />
      ...
      <div class="row mt-4">
        <div class="form-group col-12">
          ...
          <textarea class="form-control" v-model="content" id="blogContent" rows="40"></textarea>
          <button class="btn btn-secondary float-right" @click="submitBlog">submit</button>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      submitBlog(event) {
        ...
        this.$axios.post(postBlogUrl, this.blog)
          .then(res => {
            console.log(res)
            if (res.status === 201) {
      ...
      submitBlogTag(item, blogId) {
        ...
        const postBlogTagUrl = this.HOST + "/api/blogtag/" + this.username + "/" + item + "/" + blogId
        this.$axios.post(postBlogTagUrl)
          .then(res => {
            console.log(res)
          })
          .catch(error => {
